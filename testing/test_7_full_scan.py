...
# For other cars:
# Swift Dzire 1.2 petrol: max_torque_nm=113, max_power_kw=61, displacement_cc=1197
...
def calculate_derived(vals, specs):
    ...
    if fuel_rate is not None:
        ...
        afr, density = (14.5, 0.832) if specs["fuel_type"] == "diesel" else (14.7, 0.745)
        fuel_rate_est = round((maf * 3600) / (afr * density * 1000), 2)
        derived["calc_fuel_rate_lph"] = fuel_rate_est
        derived["calc_fuel_l100km"] = (
            round((fuel_rate_est / speed) * 100, 2) if speed is not None and speed > 2 else None
        )
    else:
